...
export default function Questions({ question, index, answer, resp, setResp, result, setResult, setBase, setPontuation, setboxQuestions}) {
    ...
    if (resp === 'question') {
        return (
            <Question index={index} indexSel={indexSel} correction={correction} setResp={setResp} question={question} questionSel={questionSel}/>
        )
    }
    if (resp === 'answer') {
        if (correction !== '') {
    ...
    if (resp === 'correction') {
        return (
            <Correction index={index} indexSel={indexSel} correction={correction} setQuestionSel={setQuestionSel} setAnswerSel={setAnswerSel} setIndexSel={setIndexSel} setResp={setResp} answer={answer} question={question}/>
        )
    }
